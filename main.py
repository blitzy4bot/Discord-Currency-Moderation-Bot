...
async def SUHelp(ctx):
    if getModLevel(ctx.author) >= 3:
        help_embed = discord.Embed(title=f"**Superuser command list**", icon_url=bot.user.avatar_url, color=Color.red())
        ...
        help_embed.add_field(name=f'```getrole```', value=f'Get a users moderation level. \n usage {bot.command_prefix}getrole mention/ID', inline=True)
...
async def helpBasic(ctx):
    ...
    icon_url=bot.user.avatar_url, color=Color.red())
...
async def helpCommandEconmonics(ctx):
    ...
    help_embed.add_field(name='```addmod```', value=f'Add a new Bot-Moderator (Bot-Admin required).\n Usage: {bot.command_prefix}addmod mention/ID', inline=True)
    help_embed.add_field(name='```delmod```', value=f'Remove a Bot-Moderator (Bot-Admin required).\n Usage: {bot.command_prefix}delmod mention/ID', inline=True)
    help_embed.add_field(name='```active```', value=f'Remove the blacklist from a user (Bot-Admin required).\n Usage: {bot.command_prefix}active mention/ID', inline=True)
    help_embed.add_field(name='```detain```', value=f'Blacklist a user so he cant use special commands (Bot-Admin required).\n Usage: {bot.command_prefix}detain mention/ID', inline=True)
...
async def showServerinfo(ctx):
    ...
    serverinfo_embed = discord.Embed(title=f"**SERVERINFO** :globe_with_meridians:", icon_url=bot.user.avatar_url, color=Color.red())
...
async def activateMember(ctx, user: discord.Member):
    if int(isActive(user)) == 0:
        if setActive(ctx.author, user) == 0 and ctx.author != user:
...
async def detainMember(ctx, user: discord.Member):
    if int(isActive(user)) == 1:
        if setInactive(ctx.author, user) == 0 and ctx.author != user:
...
async def checkActivityStatus(ctx, user:discord.Member):
    if getModLevel(ctx.author) >= 2:
        if int(isActive(user)) == 1:
            ...
            await ctx.send(f"{user.mention} is banned from using special Bot commands.")
    else:
        await ctx.send(f"{ctx.author.mention} You dont have permission (Bot-Admin) or the target is equal or above you.")
    ...
    if int(isActive(ctx.author)) == 1:
        ...
        if userBalance >= change_nickname_price:
            if getModLevel(user) >= 1:
                ...
                print(f"{ctx.author.name}/{ctx.author.id} tried to change nickname of {user.name}/{user.id}.")
            else:
                await ctx.message.delete()
